...
### 4a. Gateway crashes overnight [RESOLVED IN v2]
- **Previous Symptom:** Agents stop responding on Telegram. `docker ps` show container "Up" but gateway dead. Process failed to sleep-infinity.
- **Resolution:** Replaced the bespoke install-once `run` script with the official Docker Image (`ghcr.io/openclaw/openclaw:latest`). The container natively delegates failure recovery to Docker's `--restart always`, allowing fully automated healing overnight.

### 4b. Missing plugin dependencies [RESOLVED IN v2]
- **Previous Issue:** `npm install -g openclaw` omitting dependencies logic.
- **Resolution:** Inheriting from official image solves all plugin omission errors natively.

### 4c. Plugin ownership mismatch [RESOLVED IN v2]
- **Previous Symptom:** Suspicious ownership blocking plugins.
- **Resolution:** Official Docker Image solves node mapping by explicitly running openclaw processes. No file ownership collision.

### 4d. `openclaw update` logic [RESOLVED IN v2]
- **What it does:** Use `oc-upgrade` to simply redownload the latest Docker image to effortlessly replace the container and instantly reboot OpenClaw!
...
1. ~~**Gateway auto-restart**~~ — *SOLVED natively via official image.*

2. ~~**Consider official OpenClaw Docker image**~~ — *SOLVED.*
